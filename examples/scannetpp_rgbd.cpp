...
void LoadImages(const std::string &directory, std::vector<std::string> &imageFilenames);
...
int main(int argc, char **argv)
{
    ...
    std::string strImageDir = std::string(argv[4]) + "/undistorted_images";
    std::cout << strImageDir << std::endl;
    LoadImages(strImageDir, vstrImageFilenamesRGB);
    for (int i = 0; i < vstrImageFilenamesRGB.size(); i++)
    {
        std::cout << vstrImageFilenamesRGB[i] << std::endl;
    }
    ...
    for (int ni = 0; ni < nImages; ni++)
    {
        ...
        imRGB = cv::imread(std::string(argv[4]) + "/undistorted_images/" + vstrImageFilenamesRGB[ni] + ".JPG", cv::IMREAD_UNCHANGED);
        cv::resize(imRGB, imRGB, cv::Size(640, 480));
        cv::cvtColor(imRGB, imRGB, CV_BGR2RGB);
        imD = cv::imread(std::string(argv[4]) + "/undistorted_depths/" + vstrImageFilenamesRGB[ni] + ".png", cv::IMREAD_UNCHANGED);
        ...
        if (imRGB.empty())
        {
            std::cerr << std::endl << "Failed to load image at: "
                      << std::string(argv[4]) << "/undistorted_images/" << vstrImageFilenamesRGB[ni] << std::endl;
            return 1;
        }
        if (imD.empty())
        {
            std::cerr << std::endl << "Failed to load depth image at: "
                      << std::string(argv[4]) << "/undistorted_depths/" << vstrImageFilenamesRGB[ni] << std::endl;
            return 1;
        }
    ...
    //     std::vector<float> dssim_vec(Traj.size(), 0.0f);
    //     std::vector<float> psnr_gs_vec(Traj.size(), 0.0f);
    ...
    //     }
    //     float dssim_avg_value = std::accumulate(dssim_vec.begin(), dssim_vec.end(), 0.0f) / Traj.size();
    //     float psnr_gs_avg_value = std::accumulate(psnr_gs_vec.begin(), psnr_gs_vec.end(), 0.0f) / Traj.size();
    //     float psnr_gsKf_avg = std::accumulate(psnr_gsKf_vec.begin(), psnr_gsKf_vec.end(), 0.0f) / psnr_gsKf_vec.size();
    //     float psnr_test_avg = (std::accumulate(psnr_gs_vec.begin(), psnr_gs_vec.end(), 0.0f) -
    //                            std::accumulate(psnr_gsKf_vec.begin(), psnr_gsKf_vec.end(), 0.0f)) /
    //                           (Traj.size() - psnr_gsKf_vec.size());

    //     std::cout << "[Render metric evaluation progress] at "<< pGausMapper->getIteration() << " Iteration: "  << idx  << std::endl;
    //     std::cout << "\tSSIM:: " << dssim_avg_value << std::endl;
    //     std::cout << "\tPSNR_GS:: \033[1;32m" << psnr_gs_avg_value << "\033[0m" << std::endl;
    //     std::cout << "\tPSNR_KF:: \033[1;32m" << psnr_gsKf_avg << "\033[0m" << std::endl;
    //     std::cout << "\tPSNR in test:: \033[1;32m" << psnr_test_avg << "\033[0m" << std::endl;
    // }
...
int ExtractNumericSuffix(const std::string& filename) {
    std::string suffix = filename.substr(filename.size() - 5, 5); // Extract the last 5 characters
    return std::stoi(suffix);
}

void LoadImages(const std::string& directory, std::vector<std::string>& imageFilenames) {
    std::filesystem::path pathImageDir(directory);

    if (std::filesystem::exists(pathImageDir) && std::filesystem::is_directory(pathImageDir)) {
        for (const auto& entry : std::filesystem::directory_iterator(pathImageDir)) {
            if (entry.is_regular_file()) {
                ...
                // Check if the file has an image extension (optional)
        ...
        // Sort filenames based on the numeric suffix extracted from the last 5 characters
        std::sort(imageFilenames.begin(), imageFilenames.end(), [](const std::string& a, const std::string& b) {
            return ExtractNumericSuffix(a) < ExtractNumericSuffix(b);
        });
    } else {
